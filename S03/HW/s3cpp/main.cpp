...
public:
    ...
    {
        m_stdNum = stdnum;
        strncpy(m_FirstName,fname,sizeof(m_FirstName)-1);
        m_FirstName[sizeof(m_FirstName)-1]='/0';
        strncpy(m_LastName,lname,sizeof(m_LastName-1));
        m_LastName[sizeof(m_LastName-1)] = '/0';
        m_CoursesPassed = 0;
    }
    ~Student()
    {
        std::cout << "Student " << m_FirstName << " " << m_LastName << " has been deleted." << std::endl;
    }
